...
- [x] 6. **AMENDED after the fact, by the reader on 2026-08-21.** The chart now draws the outbound leg on a round trip too, with a note saying so, and the four statements agree on *that* leg. Re-checked: Climb 134 ft, figcaption 134 ft up / 8 ft down, aria-label "134 ft of climb ... over 1.2 mi", scrubber max 1860 m, announcement 134 ft — while the Distance stat above still reads 2.3 mi for the whole outing. Original text: On a round trip the Climb stat, the figcaption totals, the `aria-label`'s distance and the scrubber's maximum all describe the same out-and-back walk, and the chart's trace returns to its starting height. Checkable by eye on one card: the Distance stat and the `aria-label` distance read the same number.
