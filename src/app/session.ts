...
export type Session = {
  ...
  /**
   * The room this session is in, or null (CONTEXT.md: **room**). Set by
   * opening a room pointer or by starting a room; the socket, the partner's
   * side and the presence all hang off `useRoom` while this is set.
   */
  room: string | null;
  /**
   * The other person's start, or null. Read-only on this device: it arrived
   * over the room and the person it belongs to is not here. Only ever set
   * from a resolved `setup` frame, so it is a preset's own entry or a
   * `partnerOrigin` whose name is a literal.
   */
  partner: Origin | null;
  /**
   * False for exactly one state: a room opened from its link before this
   * reader has chosen a start.
   *
   ...
   * warmed, Spin is not pressable, and nothing is sent into the room** -
   * because `origin` is still DEFAULT_ORIGIN, Monroe Park, which has nothing
   * to do with this reader, and answering a stranger's question from it is
   * the same lie as the circle. The first `origin` action of any kind sets it
   * true forever.
   */
...
export type Action =
  ...
  | { type: "frame" }
  | { type: "dismissShared" }
  /** This device minted a room and is its first seat. */
  | { type: "enterRoom"; room: string }
  /** The other side shared, moved or withdrew its start. */
  | { type: "partner"; origin: Origin | null }
  | { type: "leaveMeet" }
  ...
  switch (action.type) {
    case "origin":
      ...
      return {
        ...
        shared: null,
        // Choosing your own start is how you answer a room's link, so it must
        // not leave the room: `room` and `partner` both survive, and moving
        // your own start later does not un-invite anybody either.
        originChosen: true,
    ...
    case "enterRoom":
      return { ...state, room: action.room, shared: null };
    /**
     * Their start, from a setup frame. Identity is preserved when nothing
     * moved, so a setup that only changed their minutes does not restart
     * their ladder's warm-up or re-frame the map.
     */
    case "partner": {
      const next = action.origin;
      const same =
        (state.partner === null && next === null) ||
        (state.partner !== null &&
          next !== null &&
          state.partner.lat === next.lat &&
          state.partner.lng === next.lng &&
          state.partner.id === next.id);
      ...
      return {
        ...
        partnerWarmed: 0,
        partnerFailure: null,
        // Their contour lands after yours without a dial commit, so the map
        // is told; a pick made against the old intersection is stale.
        pickedId: null,
        spinning: false,
        spinAborted: false,
        routeAttempt: 0,
    ...
    /**
     * Drop the other person and go back to being one app. The room goes
     * with them: this is the solo escape hatch from a full or closed room as
     * much as it is leaving a live one.
     *
 ...
 *
 * A **room pointer** restores nothing but the room: no origin, no settings,
 * because the link carries none. `originChosen` goes false so opening the
 * link costs the reader nothing until they choose — unless `restored` names
 * the start this very device already shared into that room, in which case a
 * reload is the same walker back again.
 *
...
export function applyShare(
  ...
  presets: readonly Origin[],
  restored: Origin | null = null,
): Session {
  ...
  if (link.room !== null) {
    return {
      ...base,
      room: link.room,
      origin: restored ?? base.origin,
      originChosen: restored !== null,
 ...
 * screen. App compares this against `shared.linkQuery` to decide whether to
 * clear the URL.
 *
 * @public - consumed by App and by `share.test.ts`.
 */
export function liveLinkQuery(state: Session, placeId: string | null): string {
  return canonicalQuery(decodeShare(encodeShare(shareInputFor(state, placeId))));
}
