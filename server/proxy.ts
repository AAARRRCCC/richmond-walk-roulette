...
function readLatLng(raw: unknown): LatLng | null {
...
function readMinutes(raw: unknown): number[] | null {
  if (!Array.isArray(raw) || raw.length === 0 || raw.length > MAX_LADDER) return null;
  const seen = new Set<number>();
  for (const value of raw) {
    if (typeof value !== "number" || !Number.isInteger(value)) return null;
    if (value < MIN_MINUTES || value > MAX_MINUTES) return null;
    seen.add(value);
  }
 ...
 */
async function callValhalla(base: string, path: string, body: unknown): Promise<Response> {
  ...
  try {
    ...
    if (typeof parsed === "object" && parsed !== null && "error" in parsed) {
      const detail: unknown = (parsed as Record<string, unknown>).error;
 ...
 */
async function isochrone(env: ProxyEnv, base: string, payload: Record<string, unknown>): Promise<Response> {
  ...
  for (let start = 0; start < minutes.length; start += limit) {
    ...
    if (Array.isArray(body.features)) features.push(...body.features);
  }
...
async function route(base: string, payload: Record<string, unknown>): Promise<Response> {
