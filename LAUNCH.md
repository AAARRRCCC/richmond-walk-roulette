...
- [ ] **The room-link check, deployed.**
      `curl -H 'Accept: text/html' '<deployed>/s?r=8XK2M4P9' | grep og:` answers
      **200** with a title naming **no place** and **no coordinate**, and is never
      cached (a room id is minted per room and never repeated).
